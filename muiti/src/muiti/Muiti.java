...
//				System.out.printf("%s\n",Mydiv.get(j));
	...
	public static String numMerge(String str){//对输出结果中的常数项进行合并
		String result = "";
		String queue = "";
		int i;
		int temp = 0;
		int flag = 0;//flag为0时表示队列中数字为常数项，flag为1时表示队列中数字为幂
		char ch;//遍历时使用的临时变量
		str = str + "+";
		for (i=0;i<str.length();i++){
			ch = str.charAt(i);
			if (((ch>='0')&&(ch<='9')&&(flag==0))||ch=='-'){//如果是数字且为常数项或减号
				queue = addQueue(queue,ch);//入队
			}
			else if (ch == '+'){
				temp = temp +Integer.valueOf(queue);//计算合并常数项
				if (i!=str.length()-1)//最后一次队列不清空
					queue = emptyQueue(queue);
				flag = 0;//修改标记为系数标记
			}
			else if (ch == '*'){
				if(queue.length()>0&&Integer.valueOf(queue)!=0){//如果队列非空
					result = result + "+" + queue + "*";//输出队列内容和乘号
				}
				else
					result = result + "*";//队列为空，输出乘号
			}
			else if((ch>='a')&&(ch<='z')){//字母
				result = result + String.valueOf(ch);	
			}
			else if (ch=='^'){//幂符号
				result = result + String.valueOf(ch);	
			...
			else if (((ch>='0')&&(ch<='9')&&(flag==1))){//如果是数字且为幂
				result = result + String.valueOf(ch);
			}
		}
		result = result + "+" + String.valueOf(temp);
		if (result.charAt(0)=='+')//截去开头的加号
			result = result.substring(1);
		return result;
	}
	...
	switch(choice)
	{
		case 1:
			...
			if (command.length()==9)
				...
				ss1 = numMerge(ss1);
				ss1 = zeroClear(ss1);//清除输出结果中的0
				System.out.printf("合并常数项结果：%s\n",ss1);
			}
			break;
		case 2:
			muiti s1 = new muiti();
